                     ...
                     const pheromone_matrix_edge & edge_pheromones, const pheromone_matrix_vertex & vertex_pheromones )
{
    ...
    do
    {
        ...
        if( vertex_hull.size() )
        {
            ...
            next_vertex= weight_choose_random_best( vertex_pathsegment_alternatives )->to;
#endif
        }
...
int main( int argc, char **argv)
{
    ...
    desc.add_options()
         ...
         ("use-local-search", boost::program_options::value<bool>(&use_local_search), "daemon can interact after each run")
             ;
